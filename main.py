...
def Layout(title, socials, *tags):
    ...
    return title, socials, (
        Header(
            A(Img(
                cls='borderCircle', alt='Imtiaz Khan', src='/public/images/profile.jpg', width='165', height='158')
                , href='/'),
            A(H2('Imtiaz Khan'), href="/"),
            P(
                A('About', href='/about'),' | ', 
    ...
    Main(*tags),
    Footer(Hr(), P(
                A('LinkedIn', href='https://www.linkedin.com/in/imtiaz-khan-54257bbb/'), ' | ',        
                A('Twitter', href='https://x.com/imtiazkhan_ds'), ' | '
            ),
            P(f'All rights reserved {datetime.now().year} @ Imtiaz Khan')
        ),
    Div(
        Div(
            H2('Search'),
...
@rt("/posts")
def get():
    duration = round((datetime.now() - datetime(2022, 9, 3)).days / 365.25, 2)
    description = f'Everything written by Imtiaz Khan for the past {duration} years.'
    posts = [BlogPostPreview(title=x["title"],slug=x["slug"],timestamp=x["date"],description=x.get("description", "")) for x in list_posts()]
